...
The function `pnorm` returns the probability that the a given value is lower than `test` under the standard normal with `lower.tail = FALSE`. When `lower.tail = TRUE`, it calculates the probability greater than `test`. Therefore, in order to get the $p$ value for the two-sided test, we need to add them together.

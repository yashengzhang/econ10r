...
```
Now the data frame is much better organized. Each cases/year is treated as a single observation, and we have two variables, `year` and `cases`. The data frame is "longer" in the sense that there are now more rows compared to before, so it is relatively longer. For another example, let us look at `table4b`:
...
```{r}
left_join(pivot_longer(table4a, c("1999", "2000"), names_to = "year", values_to = "cases"), pivot_longer(table4b, c("1999", "2000"), names_to = "year", values_to = "population"))
```

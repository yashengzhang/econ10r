...
*Note: This file is a continuation of the R Markdown file from week 1. This is because we have materials from week 1 that are not finished yet.*
...
However, if you run this function over and over, it will keep returning the king of spades to you. This is because neither does R know that king of spades should be taken away after being selected, nor do we shuffle the deck afterwards. It is more difficult to do the former, so let's try the latter for now.
...
We can even combine the two steps and skip the creating of a new data frame (deck2):
...
Now that we have the package installed, you can exclude it from your codes, as you do not need (or want) to install it every time you run the codes - it just needs to be installed once.
...
Another common problem you will encounter is that the data you import have columns names that are not of variables, but values of a variable. Let us import a default dataset from `tidyverse` called `table4a`:

```{r}
table4a
```

This shows the number of tuberculosis (TB) cases in three countries across two years. Generally, this isn't what you want - year is supposed to be a variable, but now the values are the variable. To deal with this, we can use the `pivot` command:
...
We want each observation to be `country-year-case-population`, but the above table clearly gives us one observation on two rows. This is when we can apply `pivot_wider`:

...
R has built-in commands to variance and standard deviations given how important they are:
...
This command does not do the subtraction for you, but it gives useful information in one word. Again, range only makes sense for certain variables, like `dep_delay` and `arr_delay`, but not others like `destination`, `carrier` or even `dep_time`.

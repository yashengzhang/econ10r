...
head(arrange(college, desc(PhD))[, c("College", "PhD", "Private")], 10)
```
...
head(select(air, date, year, month, day))
```

Alternatively, using functions from the `lubridate` package:
...
ggplot(data = air_lag, aes(x = AQI_lag, y = AQI_mean)) +
  ...
  labs(title = "Correlation Between AQI Across Two Consecutive Days",
       x = "AQI", y = "AQI Tomorrow Conditional on AQI Today")
```

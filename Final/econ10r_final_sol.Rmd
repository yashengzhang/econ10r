...
Regress log GDP per capita on measures of property protection. Take into account the potential presence of heteroskedasticity errors. What do you observe?
...
Let's do IV (two stage least squares) by hand. In the first stage, regress `prot` against `logmort`. Use the `predict` command to create a new variable that contains the predicted values — call this `prothat`. Now regress `lgdp` on `prothat`.

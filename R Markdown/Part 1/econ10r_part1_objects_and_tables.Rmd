---
title: "Econ 10R Part 1: Objects and Tables"
author: "Luke Zhang"
date: August 11, 2021
output:
  html_document:
...
*This file contains all the materials on the topics of objects and tables, and acts as an introduction to the basic operations of R. It covers the classes from week 1 Tuesday to week 2 Tuesday (inclusive). Last update: August 11, 2021*
...
Notice that the number `32` can be a character or a number. A simple rule is that when R displays something with quotation mark around them, then they are character strings rather than numbers.
...
So we are told that 1+1 is not greater than 2 by R. What a surprise. These two logical operators are very important later on in the course, when you have to specify whether to enable or disable certain options. We will see in another example later.
...
Clearly there aren't any. Now we can assign names to `die`. The way to do it is to create a vector of characters that you wish to name the values of `die`, then assign them to the output of `names(die)`:
...
Matrices store data in two-dimensional array, same as a matrix in linear algebra. We can create a matrix based on the atomic vector `die` we already have. We have to tell R how many rows or columns we want for this matrix:
...
This `m` matrix we created is two (row) by three (column) in dimension, as can be seen through the command `nrow = 2`. We can also see that the order R uses to fill out the matrix is to complete column first. This can be changed by a simple command:
...
It is easy to see that `list` is a great way to group things together in R as there is no restriction on what it can contain - elements in a list do not need to have the same type.

## Data Frame

Data frame is one of, if not the most, powerful ways to store data in R. You can think of a data frame as an Excel spreadsheet as they are both two dimensional. A vector will become a column in a data frame. We can create a simple data frame by hand to see how it works:
...
Creating data frames by hand is not practical if you have even just a little bit more data. Not only is it too time consuming, but you will inevitably make typos and mistakes. Moreover, the data we analyze are usually from external sources. It is then only natural to learn how to load downloaded data into R, deal with them, and then save them as an output.
...
read.csv("deck.csv")
```
Simple enough. However, we cannot do anything with it yet as we only told R to read the dataset, not store it. To do so, we can create a new data frame and load the data into it:
...
So far we have learned several ways of structuring data in R, and we have a fully loaded dataset, `deck`. Now we can play around the dataset a little bit more.
...
The command `head` here simply makes R show the first few rows only so it takes up a little less space on the display. Since we are working with the first few rows, it does not affect us too much.

## Deal Cards with Functions

Now we know how to extract a certain card, we can write a function to deal a card. We can simply ask R to return the one on the top of the deck:
...
However, if you run this function over and over, it will keep returning the king of spades to you. This is because neither does R know that the king of spades should be taken away after being selected, nor do we shuffle the deck afterwards. It is more difficult to do the former, so for now let's just try the latter.

## Shuffle Cards with Functions

Before we jump straight to shuffling the deck, let us use what we learned so far to reorganize the deck the way we want. Suppose I want to move the second card from top (second row) to the first and keep everything else unchanged:
...
head(deck[random, ])
```
Now we can shuffle and create a new deck using the `shuffle` function:
...
We can even combine the two steps and skip the creation of a new data frame (`deck2`):
...
A quick note on how to reference a particular variable (column) in a data frame or a list. Usually in economics, we assign names to variables. We can use the variable name and the dollar sign as reference. For example, if I wish to extract the first column of the deck (the column that has the face of the cards), we can simply write:
...
For lists, we can do the same. Recall `list1` we created earlier. We can assign names to each of its elements and refer to them using the dollar sign:
...
```{r}
die[c("one", "three", "five")] <- c(1000, 3000, 5000)
die
```
...
You can also add a new variable to a data frame in the same way, remember to use the dollar sign. Recall that `deck` is the ordered deck and `deck2` is the shuffled deck:
...
```
Once you understand the logic of these operations, it is very easy to make up your own "conditions". There are other symbols like `&`, `!` and `|` that can be very useful when you make up conditions, and we will see them in another example later.
...
Now that we have the package installed, you can exclude it from your codes, as you do not need (or want) to install it every time you run the codes - it just needs to be installed once. A better way is to write `install.packages()` in R console directly and not in your R Markdown file.
...
You should keep the above line of code in your scripts as unlike installing packages, every time you open and run the R Markdown file, you will need to tell R to load the library. For the conflicts you might see, this is because some functions in `tidyverse` share the same name as basic functions in R. If you want to use the base functions of R, you need to write `stats::filter()` instead of `filter()`, as the function with the same name from `tidyverse` is masking the base function.
...
We already learned how to select certain observations from a dataset. However, it is usually the case that this is more than one way to do the same thing. Here we can make use of the `filter` command to do the same thing in a simpler way. Suppose we wish to select all the flights on the day of Jan 1st:

```{r}
filter(flights, month == 1, day == 1)
```

Now only flights from Jan 1st appear and the sample size shrinks to 842, a good start. Comparing to using `[]` to subset the data, `filter` is a little bit more intuitive and can do things more easily. Right now we only have the data shown to us but not stored in objects, so to do that, we can assign the selected data to a new data frame:

```{r}
jan1 <- filter(flights, month == 1, day == 1)
```

You can easily click it in the environment and view it. There are other inclusion operators you can use to filter the data. For example, if you wish to include flights in November or December, we can write:
...
```
In our data this does not change much as this is the original way the dataset was arranged. We can also arrange the flights based on their departure delay. R defaults to sorting by ascending order, but you can switch to descending order if you wish by doing this:
...
We now have 336,776 observations but only three variables. The `select` command is very useful for truncating datasets if you just want to get rid of certain variables. There are other options you can use. For example, you can pick all the variables between year and day (inclusive) by doing the following:
...
Now suppose we want to know the time gained in air (departure delay time minus arrival time) and the average speed of the flight:
...
Two new variables are created and appended to the end of the data frame. You can create other variables in similar fashion, and there are also many operations you can do on existing variables that can generate new variables (e.g., leads and lags). Obviously we cannot cover everything here, but learning how to add variables in an important step.
...
I included a new variable called `count = n()` here. This gives you the number of observations that you are doing your operation (in our case, `mean()`) on. So for example, on January 1st, we are averaging the delay of 842 flights. This number is not very useful to use right now, but in cases where the number of observations in a group is small enough to affect the outcome, knowing the number would be useful. We will come across examples in the future.
...
You can also use the `group_by` function to mutate and filter the data frame. For example, suppose we wish to find the worst offenders of arrival delays by day, we can do the following:
...
```{r}
population_dest <- filter(group_by(flights, dest), n() > 365)
population_dest
```
...
Another common problem you will encounter is that the data you import have column names that are not of variables, but values of a variable. Let us import a default dataset from `tidyverse` called `table4a`:

```{r}
table4a
```

This shows the number of tuberculosis (TB) cases in three countries across two years. Generally, this isn't what you want - year is supposed to be a variable, but now the values are the variables. To deal with this, we can use the `pivot` command:

```{r}
pivot_longer(table4a, c("1999", "2000"), names_to = "year", values_to = "cases")
```
Now the data frame is much better organized. Each country-year is treated as a single observation, and we have two variables, `year` and `cases`. The data frame is "longer" in the sense that there are now more rows compared to before, so it is relatively longer. For another example, let us look at `table4b`:

```{r}
table4b
```

This is the population of each country, instead of TB cases. We can do exactly the same thing to `table4b`:

```{r}
pivot_longer(table4b, c("1999", "2000"), names_to = "year", values_to = "population")
```

Now we can combine the two data frames into a single one:
...
This is another form of mutation that we frequently use - combining two data frames based on keys (variables). The `left_join` command keeps all the rows in the first data frame, but you can search `left_join` in R to get more commands that do other things.

Now we have seen examples of `pivot_longer`, you are probably wondering what does `pivot_wider` do. Note how `table4a` and `table4b` have one observation fitted into one row but some of the values are being used as column names? Well, `pivot_wider` deals with the opposite case, when one observation is scattered across many rows. Let us look at `table2` from `tidyverse`:

```{r}
table2
```

We want each observation to be `country-year-case-population`, but the above table clearly gives us one observation in two rows. This is when we can apply `pivot_wider`:

```{r}
pivot_wider(table2, names_from = "type", values_from = "count")
```

We grabbed the column names from the `type` variable, and the values from the `count` variable. Wider in this context means that we are shortening the number of rows, so relatively speaking it is becoming wider.

# Conclusion

For the first part of the course, you are introduced to objects in R and many of the basic functions and commands that will be very useful when dealing with real data. By this point, you should feel confident in trying, on your own, to install and load packages, load and save external datasets, create vectors and data frames by hand, and make some changes to them. As always, the commands and functions covered above are no where near everything that R has to offer, so learning to search for useful syntax and commands over the Internet is also very important and is something that you should try on your own.